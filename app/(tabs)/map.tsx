        ...
        try {
          ...
          for (const review of userReviews) {
            if ((review as any).restaurantName) {
              const county = await getCountyFromAddress((review as any).restaurantName);
              console.log('County found for restaurant:', (review as any).restaurantName, '-> County:', county);
              if (county) {
                counties.add(county);
              }
            }
          ...
          console.log('Available counties in map:', Object.keys(californiaCounties));
      ...
      try {
        ...
        if (data.results && data.results.length > 0) {
          ...
          if (countyComponent) {
            ...
            console.log('Found county:', countyName, 'for restaurant:', restaurantName);
            return countyName;
          }
        } else {
        ...
        <View className="flex-1" style={{ backgroundColor: "transparent" }}>
          ...
          <Ionicons name="settings-sharp" size={32} color="#723D46" />
            ...
            <View className="flex justify-center items-center">
              <View>
                <Svg width={400} height={500} viewBox="0 0 800 600"
  preserveAspectRatio="xMidYMid meet">
                  <G>
                    {Object.entries(californiaCounties as { [key: string]: string }).map(([countyName, pathData]) => {
